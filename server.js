...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS teacher_status (
      ...
      PRIMARY KEY (teacher_name, status_date)
    );
  `);
...
// Shared logic: clear a given day's scans and notify connected dashboards.
// Used by both the manual "Clear Today" button and the automatic 5pm job.
async function clearScansForDate(date) {
...
app.delete('/api/scans', async (req, res) => {
  try {
    const date = req.query.date || todayStr();
    const deleted = await clearScansForDate(date);
...
// ---------- API: "Done for Today" — a teacher marks themselves finished scanning ----------
app.post('/api/teacher-done', async (req, res) => {
  try {
    ...
    const status_date = todayStr(); // IST date — status naturally resets each new day

    await pool.query(
      `INSERT INTO teacher_status (teacher_name, status_date, done_at)
       VALUES ($1, $2, $3)
       ON CONFLICT (teacher_name, status_date) DO UPDATE SET done_at = EXCLUDED.done_at`,
      [teacherName, status_date, done_at]
    );

    io.emit('teacher-done', { teacher_name: teacherName, done_at });
    res.json({ success: true });
  } catch (err) {
    console.error('Teacher-done error:', err);
    res.status(500).json({ error: 'Failed to record status' });
  }
});

app.get('/api/teacher-done', async (req, res) => {
  try {
    const status_date = todayStr();
    const { rows } = await pool.query(
      'SELECT teacher_name, done_at FROM teacher_status WHERE status_date = $1 ORDER BY done_at ASC',
      [status_date]
    );
    res.json(rows);
  } catch (err) {
    console.error('Teacher-done fetch error:', err);
...
// ---------- Automatic daily clear at 5:00 PM IST ----------
// Only clears today's SCANS (latecomer log) — the student master list is never touched.
cron.schedule('0 17 * * *', async () => {
  try {
    const date = todayStr();
    const deleted = await clearScansForDate(date);
    console.log(`Auto-clear (5pm IST): removed ${deleted} scan(s) for ${date}`);
  } catch (err) {
